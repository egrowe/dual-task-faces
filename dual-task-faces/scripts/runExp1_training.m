...
for b = 1:nBlocks
    ...
    % Randomize Trials (shorten nTrials for central single-task)
    ...
    Data(b).p_performance_MvNM = [];
    Data(b).estim_pSOA_MvNM = [];
    
    % Store performance and SOAs
    switch cond
        case 0
            Data(b).p_performance_MvNM = sum([TR(:).p_response])/nTrials;
            Data(b).estim_pSOA_MvNM = QuestMean(p);           
        case 1
    ...
    %Show participant performance between blocks
    switch cond
        case 0
        showPerformance = Data(b).p_performance_MvNM;
        showPerformance = showPerformance*100;
        case 1
        showPerformance = Data(b).c_performance;
        showPerformance = showPerformance*100;
        case 2
        showPerformance = Data(b).p_performance;
        showPerformance = showPerformance*100;
        case 3
        showPerformance = Data(b).c_performance;
        showPerformance = showPerformance*100;
        showPerformanceB = Data(b).p_performance;
        showPerformanceB = showPerformanceB*100;
    end
            
    if cond == 3
        DrawFormattedText(Cfg.windowPtr, ['Performance in last block Central = ' num2str(showPerformance) '% and Peripheral = ' num2str(showPerformanceB) '%'] , 'center', Cfg.yCentre, [255 255 255], 80, [], [], 2);
    else
        DrawFormattedText(Cfg.windowPtr, ['Performance in last block = ' num2str(showPerformance) '%'], 'center', Cfg.yCentre, [255 255 255], 80, [], [], 2);
    end
    
    DrawFormattedText(Cfg.windowPtr, 'Click to continue.', 'center', Cfg.yCentre+200, [255 255 255], 80, [], [], 2);
...
Screen('DrawText', Cfg.windowPtr, 'Press Spacebar to Exit', Cfg.xCentre-120, Cfg.yCentre+100, [255 255 255]);
...
if cond == 0
    ...
    instr2 = 'Please focus on the centre of the screen and decide whether the faces presented to you in the PERIPHERY ARE a GENDER MATCH or NO MATCH.';
    
    DrawFormattedText(Cfg.windowPtr, 'Peripheral Task: Match vs No Match', 'center', 400, [255 255 255], 80, [], [], 2);
...
elseif cond == 2
    ...
    DrawFormattedText(Cfg.windowPtr, 'Peripheral Task: Similarity Ratings', 'center', 400, [255 255 255], 80, [], [], 2);
    DrawFormattedText(Cfg.windowPtr, instr2, 'center', 500, [255 255 255], 80, [], [], 2);
    DrawFormattedText(Cfg.windowPtr, '<<Click to begin with the first trial>>','center', 750);
    Screen('Flip', Cfg.windowPtr, [], []);
    WaitSecs(.3);
